...
public class JailVote extends TargetedVote {
	...
	protected void votePass() {
		...
		if (ess != null) {
			...
			MiscUtils.permBroadcast(CarbonVoteModule.VMSG_PERM, plList, Module.getMsg("carbonvote", "jail-vote-passed", true, rep));
			if (target.isOnline()) ((Player) target).sendMessage(Module.getMsg("carbonvote", "jail-message", false, rep));
		}
	}
