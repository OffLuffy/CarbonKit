...
public class MuteVote extends TargetedVote {
	...
	protected void votePass() {
		...
		com.earth2me.essentials.User eUser = ess.getUser(target);
		eUser.setMuted(true);
		...
		MiscUtils.permBroadcast(CarbonVoteModule.VMSG_PERM, MiscUtils.quickList((Player)target),
				MiscUtils.massReplace(CustomMessage.CV_MUTE_VOTE_PASSED.pre(), rep));
		if (target.isOnline())
			((Player)target).sendMessage(MiscUtils.massReplace(CustomMessage.CV_MUTE_MESSAGE.pre(), rep));
	}
