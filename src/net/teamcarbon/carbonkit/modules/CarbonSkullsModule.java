		...
		if (e.getAction().equals(Action.RIGHT_CLICK_BLOCK)) {
			if (e.getClickedBlock().getState() instanceof Skull) {
				if (perm(e.getPlayer(), "check")) {
					...
					if (sb.hasOwner()) {
						getSkulls.put(e.getPlayer(), sb.getOwningPlayer().getName());
						HashMap<String, String> rep = new HashMap<>();
						rep.put("{SKULLOWNER}", sb.getOwningPlayer().getName());
						if (!perm(e.getPlayer(), "getskull", "skull.free"))
							e.getPlayer().sendMessage(CustomMessage.CS_SKULL_CHECK.pre(rep));
						if (perm(e.getPlayer(), "skull.free") || getConfig().getDouble("price", 5000) <= 0) {
							e.getPlayer().sendMessage(CustomMessage.CS_GET_SKULL_FREE.pre(rep));
						} else if (perm(e.getPlayer(), "getskull")) {
							rep.put("{PRICE}", getConfig().getDouble("price", 5000) + "");
							e.getPlayer().sendMessage(CustomMessage.CS_GET_SKULL.pre(rep));
						}
					}
				}
			}
