package net.teamcarbon.carbonkit.commands.Firework;

import net.teamcarbon.carbonkit.CarbonKit;
import net.teamcarbon.carbonkit.modules.FireworkModule;
		...
		if (args.length < 1 || MiscUtils.eq(args[0], "help")) {
			...
			if (MiscUtils.eq(args[0], "preset", "pre")) {
					...
					if (MiscUtils.eq(args[1], "list", "l", "view", "v")) {
						...
						if (presets.isEmpty()) {
							...
							if (paginate) {
								...
								if (!targeted && args.length > 2 && MiscUtils.isInteger(args[2]))
									page = MiscUtils.normalizeInt(Integer.parseInt(args[2]), 1, pages);
								else if (targeted && args.length > 3 && MiscUtils.isInteger(args[3]))
									page = MiscUtils.normalizeInt(Integer.parseInt(args[3]), 1, pages);
						...
						if (paginate) {
							pages = (presets.size() / PER_PAGE) + (presets.size() % PER_PAGE != 0 ? 1 : 0);
							if (args.length > 2 && MiscUtils.isInteger(args[2]))
								page = MiscUtils.normalizeInt(Integer.parseInt(args[2]), 1, pages);
							from = (page - 1) * PER_PAGE;
							to = (PER_PAGE * page) - 1;
						}
				...
				if (args.length > 1) {
					if (MiscUtils.eq(args[1], "hand")) {
						...
						if (((Player)sender).getItemInHand().getType().equals(Material.FIREWORK)) {
							...
							power = MiscUtils.normalizeInt(power, 1, 6);
						...
						if (MiscUtils.isInteger(args[2])) {
							power = MiscUtils.normalizeInt(Integer.parseInt(args[2]), 1, 4);
							powerSet = true;
						}
				...
				***************************************************************************************************** */
				...
				if (args.length > 1) {
					if (MiscUtils.isBoolean(args[1])) {
						boolean b = MiscUtils.toBoolean(args[1]);
						if (args.length > 2) {
							if (MiscUtils.perm(sender, CKF_PERM + "arrow.toggle.others")) {
								if (Bukkit.getPlayer(args[2]) != null) {
									FireworkModule.setArrowsEnabled(Bukkit.getPlayer(args[2]), b);
									if (b) {
							...
							if (MiscUtils.perm(sender, CKF_PERM + "arrow.toggle")) {
								FireworkModule.setArrowsEnabled((OfflinePlayer)sender, b);
								if (b)
						...
						if (args.length > 2) {
							...
							if (Bukkit.getPlayer(args[2]) != null) {
								FireworkModule.toggleArrowsEnabled(Bukkit.getPlayer(args[2]));
								if (FireworkModule.hasArrowsEnabled(Bukkit.getPlayer(args[2]))) {
									Bukkit.getPlayer(args[2]).sendMessage(CustomMessage.FW_ARROW_ON_SELF.pre());
									sender.sendMessage(CustomMessage.FW_ARROW_ON_OTHER.pre());
								} else {
									Bukkit.getPlayer(args[2]).sendMessage(CustomMessage.FW_ARROW_OFF_SELF.pre());
							...
							FireworkModule.toggleArrowsEnabled((OfflinePlayer)sender);
							if (FireworkModule.hasArrowsEnabled((OfflinePlayer)sender))
								sender.sendMessage(CustomMessage.FW_ARROW_ON_SELF.pre());
							else
								sender.sendMessage(CustomMessage.FW_ARROW_OFF_SELF.pre());
						}
						...
						if (args.length > 2) {
							...
							if (pl != null) {
								...
								FireworkModule.setArrowEffects(pl, fx);
								...
								FireworkModule.setArrowEffects((OfflinePlayer) sender, fx);
				...
				if (args.length > 1) {
					if (MiscUtils.isBoolean(args[1])) {
						...
						if (args.length > 2) {
							if (MiscUtils.perm(sender, CKF_PERM + "snowball.toggle.others")) {
								if (Bukkit.getPlayer(args[2]) != null) {
									FireworkModule.setSnowballEnabled(Bukkit.getPlayer(args[2]), b);
									if (b) {
							...
							if (MiscUtils.perm(sender, CKF_PERM + "snowball.toggle")) {
								FireworkModule.setSnowballEnabled((OfflinePlayer) sender, b);
								if (b)
						...
						if (args.length > 2) {
							...
							if (pl != null) {
								...
								FireworkModule.setSnowballEffects(pl, fx);
								pl.sendMessage(CustomMessage.FW_EFFECT_SET_OTHER.pre());
								sender.sendMessage(MiscUtils.massReplace(CustomMessage.FW_EFFECT_SET_SELF.pre(), rep));
							} else {
								if (!MiscUtils.perm(sender, CKF_PERM + "snowball.preset")) {
								...
								FireworkModule.setSnowballEffects((OfflinePlayer) sender, fx);
								sender.sendMessage(CustomMessage.FW_EFFECT_SET_SELF.pre());
							}
						} else {
